...
#### F.7.1 Briefs — the ModelRenderer as built

The ModelRenderer above assumes an explanation attached to a ChangeRecord. Two shipped surfaces
have no change record, so they are served by **briefs**: shared per-instrument copy, rendered by
the worker into `instrument_briefs` (migration 0008) and read by the API.

| Surface | Subject | Content |
|---|---|---|
| First view | an instrument the user has never opened | what the company does, and where the price sits against its own baseline |
| Returning view | a previously-seen instrument | the price now versus the price before |

Both are cached by `(instrument_id, renderer_version, session_date, window_bucket)`. Keying on
session date rather than live price means an intraday tick cannot invalidate the cache; bumping
`renderer_version` produces new rows rather than mutating history, as elsewhere.

**Personal timing (amends "no personal timing").** The returning view is personal timing by
definition, so the original blanket rule is refined rather than kept: the model sees an
*anonymised elapsed-time bucket* — `FIRST_VIEW | D1 | D2 | W1 | M1`, calendar-day lookbacks of
1, 2, 7 and 30 days — and nothing else. Days rather than trading sessions, because the span a
brief describes is the time since the user last opened the stock, which is wall-clock. The API
rounds a checkpoint age down to a bucket and reads the shared row for it; no user id, watchlist,
baseline, or exact elapsed time reaches the worker or the model. Generation therefore scales with
instruments × buckets, never users × instruments. The exact "since you last checked 3 days ago,
it has moved 2.1%" remains template-generated by the API from the user's own checkpoint.

Because bucket and checkpoint age differ, a brief states its own window explicitly ("over the last
2 days"); it is context beside the personal clause, never a restatement of it.

**Causality (unchanged, and reaffirmed).** The validator still rejects causal phrasing outright.
Grounded attribution to an `EARNINGS` event or a corporate action was considered and *not* adopted:
the shipped product surface is "price now vs. price before", and the system ingests no news, so any
cause the model could reach for beyond those two rows would be invented.

**What the validator enforces** (`worker/src/explanation/validator.ts` — the prompt asks, only this
makes it true): every numeric literal in the output must appear in the fact bundle, compared
numerically so display formatting is not a false rejection; no advisory, predictive, or causal
phrasing; no second person; no trading-session vocabulary, since briefs measure days; no
spelled-out numeral used as a quantity, which would otherwise evade a digit scan; bounded length
and sentence count. Rejection is not an error — it selects the
deterministic template, which is always correct. The validator is biased toward false rejections
on purpose.

**Timeout (amends "~1.5s").** The 1.5s bound assumed a synchronous render on the read path. Briefs
are generated in a background `render_brief` job: on a cache miss the API enqueues one and serves
the template immediately, so no user ever waits on the model. The timeout only bounds a stuck job,
and is configured at 10s.

**Credential ownership.** `GEMINI_API_KEY` belongs to the worker, and the API must never call a
model. This does not follow from the Alpaca rule — Gemini is not a market-data provider — but from
two others: everything the model writes is shared per-instrument content, and a user-facing read
must not block on an outbound generative call. The key is optional; absent, every brief is the
deterministic template and nothing else about the product changes.

Change-record `shared_explanation` remains template-only. Extending the ModelRenderer to it is
open work, not a decision taken here.
